...
class V6_Hybrid_Optimized_ETH(IStrategy):
    INTERFACE_VERSION = 3
    ...
    # 하이퍼옵트 파라미터 선언 (이더리움 특화형 최종 Optimized)
    adx_long = IntParameter(10, 40, default=20, space='buy', optimize=True)
    adx_short = IntParameter(15, 45, default=22, space='buy', optimize=True)
    rsi_long = IntParameter(40, 70, default=42, space='buy', optimize=True)
    rsi_short = IntParameter(30, 60, default=45, space='buy', optimize=True)
    prediction_threshold = DecimalParameter(0.005, 0.050, default=0.008, space='buy', decimals=3, optimize=True)
    prediction_short_threshold = DecimalParameter(0.002, 0.030, default=0.006, space='buy', decimals=3, optimize=True)
    ...
    # 레버리지 파라미터 분리 (이더리움 특화 비대칭 레버리지 최종)
    leverage_long = IntParameter(1, 10, default=9, space='buy', optimize=True)
    leverage_short = IntParameter(1, 10, default=7, space='buy', optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (dataframe['adx'] > adx_long_val) &
                (dataframe['tema'] > dataframe['ema200'])
            ),
            'enter_long'] = 1

        # Short Entry: 액티브 헤지 숏 필터
        dataframe.loc[
            (
                (dataframe['do_predict'] == 1) &
                (dataframe['DI_ratio'] < di_limit) &
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: datetime, current_rate: float,
        ...
        if dataframe is None or dataframe.empty:
            ...
            if dataframe is None or dataframe.empty:
                return None
        try:
            # 1. 숏 포지션 전용 초고속 이익 보존 장치 (trailing profit exit)
            ...
            logger.error(f"Error in custom_exit for V6_Hybrid_Optimized_ETH: {e}")
        return None
