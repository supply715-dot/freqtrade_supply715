...
def get_latest_results(n=2):
    # RESULTS_DIR ?êÏÑú ÏµúÍ∑º ?åÏùº nÍ∞?Ï∞æÍ∏∞
...
def simulate_portfolio(btc_trades, eth_trades):
    # ?¨Ìä∏?¥Î¶¨???µÌï© ?±Í≥º ?úÎ??àÏù¥??(10,000 USDT Í∏∞Ï? Î≥µÎ¶¨)
    # ???∏Î†à?¥Îìú Î≥ëÌï© Î∞??úÍ∞Ñ???ïÎ†¨
    ...
    # ?úÍ∞Ñ ???ïÎ†¨ (?úÍ∞Ñ??Í∞ôÏ? Í≤ΩÏö∞ close ?¥Î≤§?∏Î? Î®ºÏ? Ï≤òÎ¶¨?òÏó¨ ?îÍ≥† ?ïÎ≥¥)
...
def main():
    ...
    # ?¨Ìä∏?¥Î¶¨???úÎ??àÏù¥??    print("Simulating unified portfolio performance...")
    port_profit_pct, port_mdd, port_profit_abs = simulate_portfolio(btc_df, eth_df)
    
    # Í≤∞Í≥º ?îÏïΩ JSON ?åÏùº ?∞Í∏∞
    results_data = {
        "ratio_45": {
            "btc": {
