...
class V7_Asymmetric(IStrategy):
    ...
    def leverage(self, pair: str, current_time: datetime, current_rate: float,
        ...
        if side == "short":
            return 3.0 # 개선안 A: 숏 포지션은 변동성 생존 마진 확보를 위해 3배로 축소
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                ...
                (dataframe['adx'] > 30) & # 개선안 B: 숏 진입 시 ADX 필터를 30 이상으로 강화
                (dataframe['fastEMA'] < dataframe['slowEMA']) &
                (not is_eth) # 개선안 C: 이더리움은 숏 비활성화 (롱 온리만 작동)
            ),
            'enter_short'] = 1
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: datetime, current_rate: float,
        ...
        if dataframe is None or dataframe.empty:
            ...
            if dataframe is None or dataframe.empty:
                return None
        try:
            # Check DI_ratio at trade entry
            idx = self._get_df_idx(dataframe, trade.open_date)
            di_ratio = dataframe.loc[idx].get('DI_ratio', 0.0)
            
            # Continuous mapping: low DI (0.0) -> -8.0% SL, high DI (0.5) -> -4.0% SL
            # Formula: dynamic_sl = -0.08 + (di_ratio * 0.08)
            dynamic_sl = -0.08 + (di_ratio * 0.08)
            dynamic_sl = max(-0.08, min(-0.035, dynamic_sl))
            
            if current_profit <= dynamic_sl:
                return "di_continuous_stoploss"
        except Exception as e:
            logger.error(f"Error in custom_exit for Hybrid V3: {e}")
        return None
        try:
            idx = self._get_df_idx(dataframe, trade.open_date)
            
            if 'atr' in dataframe.columns:
                atr_val = dataframe.loc[idx, 'atr']
            else:
                import talib.abstract as ta
                atr_val = ta.ATR(dataframe, timeperiod=14).loc[idx]
                
            entry_rate = trade.open_rate
            dynamic_sl = - (atr_val * 2.0) / entry_rate
            dynamic_sl = max(-0.08, min(-0.035, dynamic_sl))
            
            if current_profit <= dynamic_sl:
                return "atr_stoploss"
        except Exception as e:
            logger.error(f"Error in custom_exit for OptB: {e}")
        return None
